...
public function getMeme(index: int) : meme = 
  switch(Map.lookup(index, state.memes))
    None    => abort("There was no meme with these index registered")
    Some(x) => x
...
public stateful function registerMeme( url' : string, name': string) =
  ...
  put(state{memes[index]=meme, memeLength = index})
...
public stateful function voteMeme(index : int) = 
  ...
  let updatedCount = meme.voteCount + Call.value
  let updatedMemes = state.memes{[index].voteCount = updatedCount}
